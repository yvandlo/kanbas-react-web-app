...
function QuestionListPreview() {
    ...
    const drawNotEditType = (question) => {
        switch (question.type) {
            ...
            case "True False":
                return <h5><label> True <input type="radio" name={question._id} /> </label>
                    <br /><label> False <input type="radio" name={question._id} /> </label> </h5>;
            default:
                ...
                {question.slots.map((slot, index) => (
                    <li key={index}>
                        {(question.type === "Multiple Choice") ?
                            <>  <label> {slot} <input type="radio" name={question._id} /></label></> : <><input type="text" /></>}
                    </li>
                ))}
            </ul>
        )
